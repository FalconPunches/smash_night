...
unsafe fn online_melee_any_init(_: &InlineCtx) {
    LOCAL_ROOM_PANE_HANDLE.store(0, Ordering::SeqCst);
    ONLINE_ARENA_PANE_HANDLE.store(0, Ordering::SeqCst);
    // Fork: treat every Quickplay/Elite session as an online match, not
    // only when the Nextendo redirect is active.  Upstream gates this
    // deliberately; this fork removes the gate.
...
pub fn is_valid_online_mode() -> bool {
    ...
    // Fork: Quickplay/Elite count as valid online modes unconditionally.
    return is_online_arena_mode()
        || is_local_online_mode()
        || is_online_quickplay_mode();
}
